...
CXWalkExtension* xwalk_extension_init(int32_t api_version) {
  return ExtensionAdapter<FilesystemContext>::Initialize();
}
...
FilesystemContext::~FilesystemContext() {
  ...
  for (it = known_file_descriptors_.begin(); it != known_file_descriptors_.end(); it++)
    close(*it);
}
...
void FilesystemContext::PostAsyncErrorReply(const picojson::value& msg, WebApiAPIErrors error_code) {
  picojson::value::object o;
  o["isError"] = picojson::value(true);
  o["errorCode"] = picojson::value((double)error_code);
...
void FilesystemContext::HandleFileSystemManagerResolve(const picojson::value& msg) {
  // FIXME(leandro): This requires specific Tizen support.
  PostAsyncErrorReply(msg, NOT_SUPPORTED_ERR);
}

void FilesystemContext::HandleFileSystemManagerGetStorage(const picojson::value& msg) {
  // FIXME(leandro): This requires specific Tizen support.
  PostAsyncErrorReply(msg, NOT_SUPPORTED_ERR);
}

void FilesystemContext::HandleFileSystemManagerListStorages(const picojson::value& msg) {
  // FIXME(leandro): This requires specific Tizen support.
  PostAsyncErrorReply(msg, NOT_SUPPORTED_ERR);
}
...
void FilesystemContext::HandleFileOpenStream(const picojson::value& msg) {
  ...
  if (fd < 0) {
    ...
    picojson::value::object o;
    o["fileDescriptor"] = picojson::value((double)fd);
...
void FilesystemContext::HandleSyncMessage(const char* message) {
  ...
  else if (cmd == "FileRealPath")
    HandleFileRealPath(v, reply);
...
void FilesystemContext::HandleFileSystemManagerGetMaxPathLength(const picojson::value& msg, std::string& reply) {
...
void FilesystemContext::SetSyncError(std::string& output, WebApiAPIErrors error_type) {
  picojson::value::object o;

  o["isError"] = picojson::value(true);
  o["errorCode"] = picojson::value((double) error_type);
  picojson::value v(o);
  output = v.serialize();
}

void FilesystemContext::SetSyncSuccess(std::string& reply, std::string& output) {
  picojson::value::object o;
...
void FilesystemContext::SetSyncSuccess(std::string& reply, picojson::value& output) {
  picojson::value::object o;
...
void FilesystemContext::HandleFileStreamClose(const picojson::value& msg, std::string& reply) {
...
void FilesystemContext::HandleFileStreamRead(const picojson::value& msg, std::string& reply) {
  if (!msg.contains("fileDescriptor")) {
    SetSyncError(reply, INVALID_VALUES_ERR);
    return;
  }
...
void FilesystemContext::HandleFileStreamReadBytes(const picojson::value& msg, std::string& reply) {
...
void FilesystemContext::HandleFileStreamReadBase64(const picojson::value& msg, std::string& reply) {
  HandleFileStreamRead(msg, reply);
  std::string base64_contents = base64::ConvertTo(reply);
  SetSyncSuccess(reply, base64_contents);
}

void FilesystemContext::HandleFileStreamWrite(const picojson::value& msg, std::string& reply) {
  if (!msg.contains("fileDescriptor")) {
    SetSyncError(reply, INVALID_VALUES_ERR);
    return;
  }
...
void FilesystemContext::HandleFileStreamWriteBytes(const picojson::value& msg, std::string& reply) {
  HandleFileStreamWrite(msg, reply);
}

void FilesystemContext::HandleFileStreamWriteBase64(const picojson::value& msg, std::string& reply) {
...
void FilesystemContext::HandleFileCreateDirectory(const picojson::value& msg, std::string& reply) {
  if (!msg.contains("path")) {
    SetSyncError(reply, INVALID_VALUES_ERR);
    return;
  }
...
void FilesystemContext::HandleFileCreateFile(const picojson::value& msg, std::string& reply) {
  if (!msg.contains("path")) {
    SetSyncError(reply, INVALID_VALUES_ERR);
    return;
  }
  ...
  std::string file_path = JoinPath(parent, path);
  if (file_path.empty()) {
    SetSyncError(reply, INVALID_VALUES_ERR);
    return;
  }

  int result = open(file_path.c_str(), O_CREAT | O_WRONLY | O_TRUNC, kDefaultFileMode);
  if (result < 0) {
    SetSyncError(reply, IO_ERR);
    return;
  }
...
void FilesystemContext::HandleFileResolve(const picojson::value& msg, std::string& reply) {
  if (!msg.contains("path")) {
    SetSyncError(reply, INVALID_VALUES_ERR);
    return;
  }
...
void FilesystemContext::HandleFileStat(const picojson::value& msg, std::string& reply) {
  if (!msg.contains("path")) {
    SetSyncError(reply, INVALID_VALUES_ERR);
    return;
  }
  ...
  o["modified"] = picojson::value((double)st.st_mtime);
  o["created"] = picojson::value((double)st.st_ctime); // ?
...
void FilesystemContext::HandleFileGetFullPath(const picojson::value& msg, std::string& reply) {
  if (!msg.contains("path")) {
    SetSyncError(reply, INVALID_VALUES_ERR);
    return;
  }
