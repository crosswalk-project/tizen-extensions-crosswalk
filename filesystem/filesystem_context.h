...
public:
  FilesystemContext(ContextAPI* api);
  ~FilesystemContext();
...
private:
  ...
  void PostAsyncErrorReply(const picojson::value& msg, WebApiAPIErrors error_code);
  void PostAsyncSuccessReply(const picojson::value& msg, picojson::value::object& reply);
  void PostAsyncSuccessReply(const picojson::value& msg, picojson::value& reply);
  void PostAsyncSuccessReply(const picojson::value& msg, WebApiAPIErrors error_code);
  void PostAsyncSuccessReply(const picojson::value& msg);

  /* Sync messages */
  void HandleFileSystemManagerGetMaxPathLength(const picojson::value& msg, std::string& reply);
  ...
  void HandleFileStreamReadBytes(const picojson::value& msg, std::string& reply);
  void HandleFileStreamReadBase64(const picojson::value& msg, std::string& reply);
  void HandleFileStreamWrite(const picojson::value& msg, std::string& reply);
  void HandleFileStreamWriteBytes(const picojson::value& msg, std::string& reply);
  void HandleFileStreamWriteBase64(const picojson::value& msg, std::string& reply);
  void HandleFileRealPath(const picojson::value& msg, std::string& reply);
  void HandleFileCreateDirectory(const picojson::value& msg, std::string& reply);
  ...
  bool CopyAndRenameSanityChecks(const picojson::value& msg, const std::string& from, const std::string& to, bool overwrite);
