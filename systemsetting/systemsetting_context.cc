...
const char* SystemSettingContext::GetJavaScript() {
  return kSource_systemsetting_api;
}
...
void SystemSettingContext::HandleSyncMessage(const char* message) {
  picojson::value v;
...
void SystemSettingContext::OnPropertyHandled(const char* reply_id, const char* value, int ret) {
